...
def upload_image_to_backend(file):
    ...
    try:
        ...
        response = requests.post(
            f"{BACKEND_URL}/upload_image/", files=files, timeout=60
        )
    ...
    if uploaded_file:
        ...
        with col1:
            if is_valid and not already_uploaded:
                ...
                if st.button(
                    "🔄 Process Again", type="secondary", use_container_width=True
                ):
                    process_image(uploaded_file)
            else:
...
def render_history_section():
    ...
    successful_uploads = sum(
        1 for entry in st.session_state.upload_history if entry["success"]
    )
    failed_uploads = total_uploads - successful_uploads
    total_size = sum(
        entry.get("file_size", 0) for entry in st.session_state.upload_history
    )
    ...
    with col2:
        st.metric(
            "Successful",
            successful_uploads,
            delta=f"{successful_uploads / total_uploads * 100:.0f}%",
        )
    with col3:
...
def render_results_section(timeout_setting=30):
    ...
    successful_uploads = [
        entry for entry in st.session_state.upload_history if entry["success"]
    ]
    ...
    with col1:
        ...
        with st.spinner("Loading overlay..."):
            overlay_bytes = get_image_from_backend(
                f"overlay/{latest['filename']}", timeout_setting
            )
        ...
        if overlay_bytes:
            try:
                ...
                st.image(
                    image, caption="Water Detection Overlay", use_container_width=True
                )
            except Exception as e:
    ...
    with col2:
        ...
        with st.spinner("Loading prediction mask..."):
            mask_bytes = get_image_from_backend(
                f"predictions/{latest['filename']}", timeout_setting
            )
        ...
        if mask_bytes:
            try:
                ...
                st.image(
                    image, caption="Water Detection Mask", use_container_width=True
                )
            except Exception as e:
                st.error(f"Display error: {e}")
        else:
