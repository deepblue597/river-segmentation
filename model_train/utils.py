...
def split_dataset(
    ...
    train_pairs, temp_pairs = train_test_split(
        data_pairs, test_size=(val_ratio + test_ratio), random_state=random_seed
    )
    ...
    print("Dataset split:")
    print(
        f"  Train: {len(train_images)} images ({len(train_images) / len(data_pairs) * 100:.1f}%)"
    )
    print(
        f"  Val:   {len(val_images)} images ({len(val_images) / len(data_pairs) * 100:.1f}%)"
    )
    print(
        f"  Test:  {len(test_images)} images ({len(test_images) / len(data_pairs) * 100:.1f}%)"
    )
