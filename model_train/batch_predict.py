...
def batch_predict_water_segmentation(
    input_folder,
    output_folder="batch_predictions",
    ...
    for image_path in tqdm(image_files, desc="Processing images"):
        try:
                    ...
                    if os.path.exists(mask_path):
                        ...
                        if true_mask_img is not None:
                            ...
                            true_mask = cv2.resize(
                                true_mask_img, (original_width, original_height)
                            )
            ...
            # Save mask
            cv2.imwrite(
                os.path.join(output_folder, f"{base_name}_mask.png"),
            ...
            cv2.imwrite(
                os.path.join(output_folder, f"{base_name}_confidence.png"),
                confidence_img,
            )
            ...
            if true_mask is not None:
                ...
                overlay[prediction_mask_resized == 1] = [
                    0,
                    0,
                    255,
                ]  # Blue for predicted water
                ...
                cv2.imwrite(
                    os.path.join(output_folder, f"{base_name}_comparison.png"),
                    composite_bgr,
                )
                ...
                dice_score = (2 * intersection) / (
                    np.sum(prediction_mask_resized) + np.sum(true_mask) + 1e-8
                )
                ...
                cv2.imwrite(
                    os.path.join(output_folder, f"{base_name}_overlay.png"), overlay_bgr
                )
    ...
    with open(summary_path, "w") as f:
        ...
        if images_with_gt > 0:
            avg_dice = np.mean(
                [r["dice_score"] for r in results if r["has_ground_truth"]]
            )
            avg_iou = np.mean(
                [r["iou_score"] for r in results if r["has_ground_truth"]]
            )
            f.write(
                f"Overall Performance (on {images_with_gt} images with ground truth):\n"
            )
    ...
    if any(r["has_ground_truth"] for r in results):
        print("🎯 Performance metrics calculated for images with ground truth")
        print("📊 Comparison overlays saved as '*_comparison.png'")
    ...
    if os.path.exists(input_folder):
        ...
        if has_masks == "y":
            ...
            if os.path.exists(mask_folder):
                ...
                print(
                    "❌ Mask folder not found! Running without ground truth comparison."
                )
