...
def predict_single_image(
    image_path, model, transform, device="cuda", keep_original_size=True
):
    """
    ...
    if keep_original_size:
        ...
        confidence_map_resized = cv2.resize(
            confidence_map, (original_width, original_height)
        )
        prediction_mask_resized = cv2.resize(
            prediction_mask, (original_width, original_height)
        )
...
def visualize_prediction(
    original_image, prediction_mask, confidence_map, save_path=None
):
    """
...
def save_prediction_masks(
    original_image, prediction_mask, confidence_map, output_dir="prediction_output"
):
    ...
    # Save binary mask
    cv2.imwrite(os.path.join(output_dir, "mask.png"), prediction_mask * 255)
...
def save_mask_to_minio(prediction_mask, kafka_message):
    try:
        ...
        print("Error saving mask to MinIO:")
        return None
...
def save_confidence_to_minio(original_image, confidence_map, kafka_message):
    try:
        ...
        print("Error saving confidence map to MinIO:")
        return None
...
def save_overlay_to_minio(original_image, prediction_mask, kafka_message):
    try:
        ...
        print("Error saving overlay to MinIO:")
        return None
...
def save_statistics_to_timescale(water_percentage, avg_confidence, kafka_message):
    try:
        print(
            f"Attempting to save statistics for image: {kafka_message.get('filename', 'unknown')}"
        )
        ...
        try:
            river_segmentation_results = Table(
                "river_segmentation_results", metadata, autoload_with=engine
            )
...
def predict_and_save(X):
    ...
    try:
        ...
        print(
            f"Water pixels: {water_pixels:,} / {total_pixels:,} ({water_percentage:.1f}%)"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Predict water segmentation on a single image"
    )
    parser.add_argument("--image", type=str, required=True, help="Path to input image")
    parser.add_argument(
        "--model",
        type=str,
        default="best_model.pth.tar",
    ...
    parser.add_argument(
        "--save-viz", type=str, default=None, help="Save visualization to file"
    )
    parser.add_argument(
        "--low-res",
        action="store_true",
    ...
    try:
        ...
        save_prediction_masks(
            original_image, prediction_mask, confidence_map, args.output
        )
        ...
        if args.visualize:
            ...
            if args.save_viz is None:
                ...
                args.save_viz = (
                    f"{args.output}/visualization_{base_name}_{timestamp}.png"
                )

            visualize_prediction(
                original_image, prediction_mask, confidence_map, args.save_viz
            )
